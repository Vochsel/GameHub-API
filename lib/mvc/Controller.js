...
class Controller extends BaseObject {
	...
	format(a_options) {
		...
		return new Promise((resolve, reject) => {
			...
			/*var controllerSrcPromise = new Promise(function (csrcResolve, csrcReject) {
				if (Utils.Valid(a_options.src)) {
					var rPath = "";
					...
					Utils.LoadFileAsync(rPath + a_options.src).then(value => {
						...
						//Compile source
						var robj = Eval(value, Utils.GH_API *//* , Object.assign(module.exports.CreateContext(), a_opts) ); */
						/*if (Utils.Valid(robj)) {
							...
							csrcReject("Could not evaluate source code: " + value);
						}
					}, reason => {
	...
	getProperPath(a_src) {
		if (!Utils.IsAbsolutePath(a_src)) {
			//TODO: Add check if path begins with /states/ etc.
			if (Utils.Valid(this.parentState))
				if (Utils.Valid(this.parentState.parentStage))
					if (Utils.Valid(this.parentState.parentStage.parentGM))
						return this.parentState.parentStage.parentGM.path + this.parentState.path;
					else
						return this.parentState.parentStage.path + this.parentState.path;
			else
				return this.parentState.path;
		} else return "";
	}
}
