...
class Controller extends GameObject {
	...
	format(a_options) {
		...
		return new Promise((resolve, reject) => {
			...
			for(var fKey in a_options) {
				if (a_options.hasOwnProperty(fKey)) {
					var fVal = a_options[fKey];
					if(Utils.IsFunction(fVal)) {
						this[fKey] = fVal;
						//this[fKey].apply(this);
					}
				}
			}
				...
				if (Utils.Valid(this.parentState.parentStage)) {
					if (Utils.Valid(this.parentState.parentStage.parentGM)) {
						...
						if (Utils.IsAbsolutePath(this.parentState.parentStage.path))
							outPath = Utils.JoinPaths(this.parentState.parentStage.path, "/../../");
						else
							outPath = this.parentState.parentStage.path;
					}
				} else {
					if (srcDir[0] === "states")
						outPath = Utils.JoinPaths(this.parentState.path, "/../../"); //TODO: Make this nicer
					else
		...
		if (Utils.Valid(this.parentState))
			if (srcDir[0] !== "states")
				if(!Utils.IsAbsolutePath(this.parentState.path))
					outPath = Utils.JoinPaths(outPath, this.parentState.path);
