...
class GameMode extends BaseObject {
	...
	format(a_options) {
		...
		return new Promise(function (resolve, reject) {
			...
			Promise.all([srcPromise, stgPromise, pLoadAllResources]).then(function () {
				resolve();
			}).catch(reason => {
				Debug.Error(reason);
				reject(reason);
	...
	start() {
		...
		this.log("Started GameMode - " + this.name);
	}
	...
	stop() {
		...
		this.log("Stoped GameMode: " + this.name);
	}
	...
	progressGameMode() {
		...
		if (nextStateIdx >= this.currentStage.states.length) {
			...
			this.log("Progressed to next Stage - " + nextStageIdx, "cyan");
			return;
		}
		...
		this.log("Progressed to next State - " + nextStateIdx, "magenta");
	}
