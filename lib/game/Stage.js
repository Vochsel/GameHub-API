...
class Stage extends EventEmitter {
    ...
    constructor(a_options) {
        ...
        this.on("load", function () {
            Debug.Log("[Stage] Loaded Stage", "cyan");
        })

        /* --- Stage Debug Info --- */
        Debug.Log("[Stage] Created Stage", "cyan");

        /* --- Format with options passed --- */
        this.format(a_options).then(value => {
            this.emit("load");
        }, reason => {
            Debug.Error("Could not format State");
            Debug.Error(reason);
        });
    }
    ...
    format(a_options) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            //Owning GM of Stage
            if (Utils.Valid(a_options.owningGM))
                self.owningGM = a_options.owningGM;
            ...
            var srcPromise = new Promise(function (srcResolve, srcReject) {
                if (Utils.Valid(a_options.src)) {
                    Utils.LoadFileAsync((Utils.Valid(self.owningGM) ? self.owningGM.path : "") + a_options.src).then(value => {
